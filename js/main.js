...
function toggle_play(){
    if(player_status == "play"){
        ...
        $("#btn_play").attr("src","/static/player/play.png");
        ...
        $("#btn_play").attr("src","/static/player/pause.png");
...
function play_access(index,time){
    ...
    $("#audio")[0].src = "/music/" + music_list[index];
...
function friend_link_server(){
    if (frend_link_status == false){
        ...
        setTimeout('$(".hxjj").css({"background-image":"url(\'/static/friend_link/hxjj.jpg\')","opacity":"0"});$(".hxjj").animate({"opacity":"1.0"},1000);',1000);
    }else{
...
function prepare_friend_link(){
    if (current_song == 1){
        if(friend_link_task_id == 0){
            ...
            friend_link_task_id = setInterval('friend_link_server()', 5500);
        }
    }else{        
        if(friend_link_task_id != 0){
...
$(function player_init(){
    ...
    $("#btn_play").bind("mouseover",function(){obvious();});
    $("#btn_play").bind("mouseout",function(){unobvious();});
    $("#btn_play").bind("click",function(){toggle_play();});
    $("#pgs_block").bind("mouseover",function(){$("#pgs_block").css("cursor","pointer");});
    $("#pgs_block").draggable({ containment: "parent" });    
    $("#pgs_block").mousedown(function(){move_pgsblock();});
    ...
    $("#audio").bind('ended',function(){play_next();});
    $("#audio").bind('play',function(){prepare_friend_link();});
